...
/// Заголовок окна
const char* g_strWindowCaption = "03 - Temporal example";
...
struct UniformLocationsPt
{
    ...
    std::unordered_map<GLuint*, std::string> bindings = {
            ...
            {&viewMatrix,"iView"},
            {&viewInverseMatrix,"iViewInverse"},
            {&projMatrix,"iProj"},
            {&projInverseMatrix,"iProjInverse"},
            {&screenSize,"iScreenSize"},
            {&time,"iTime"},
            {&previousFrame,"iPreviousFrame"},
            {&previousViewProjMatrix,"iPreviousViewProj"},
    };
};
...
/// Основные кадровые буферы (поочередно сменяются)
gl::FrameBuffer* g_frameBuffers[2] = {nullptr, nullptr};
/// Индексы кадровых буферов для записи и для чтения (при рендеринге используется вспомогательный буфер прошлого кадра)
unsigned g_frameBufferIdxCurrent = 0;
unsigned g_frameBufferIdxPrevious = 1;
 ...
 * \param previousViewProj Матрица вида-проекции предыдущего кадра (для темпоральной репроекции)
 * \details Полученный кадровый буфер будет использован проходом постпроцессинга
 */
void RenderPrimaryQuad(glm::mat4& previousViewProj);
...
/**
 * \brief Изменение индексов буфера чтения и буфера записи
 * \details В данной реализации прогрессивной трассировки используются 2 буфера кадра, которые сменяются в каждом кадре
 * Один буфер используется как целевой буфер рендеринга, другой (хрянящий результат прошлого кадра) как вспомогательный
 */
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        g_camera->setAspect(static_cast<float>(clientRect.right)/static_cast<float>(clientRect.bottom),false);
        g_camera->setFov(90.0f);
        ...
        P::MaterialInfo light = {P::eLightEmitter, glm::vec3(5.0f)};
        ...
        // Матрица вида-проекции предыдущего кадра
        glm::mat4 previousFrameViewProj = glm::mat4(1.0f);
        ...
        while (true)
        {
            ...
            RenderPrimaryQuad(previousFrameViewProj);
            // Пост процессинг
            RenderFinalQuad(clientRect.right,clientRect.bottom);
            // Смена кадровых буферов
            SwitchFrameBuffers();
...
void InitOpenGl(unsigned int screenWidth, unsigned int screenHeight)
{
    ...
    /// Шейдеры
    {
        ...
        auto vsSource = LoadStringFromFile("../Shaders/03_Temporal/common.vert");
        auto fsPtSource = LoadStringFromFile("../Shaders/03_Temporal/path_tracing.frag");
        auto fsPostSource = LoadStringFromFile("../Shaders/03_Temporal/post_process.frag");
    ...
    {
        // В каждом кадре есть 2 буфера - целевой, и вспомогательный
        // Вспомогательный буфер хранит итоговый результат из прошлого кадра, данный результат усредняется с текущим результатом выхода шейдера
        // При этом буферы меняются местами в кажом кадре

        // Иницализация первичного буфера
        g_frameBuffers[0] = new gl::FrameBuffer(screenWidth,screenHeight);
        g_frameBuffers[0]->addTextureAttachment(GL_RGBA32F,GL_RGBA,GL_COLOR_ATTACHMENT0,false);
        if(!g_frameBuffers[0]->prepareBuffer({GL_COLOR_ATTACHMENT0})) throw std::runtime_error("OpenGL framebuffer initialization failed");

        // Инициализация вторичного буфера
        g_frameBuffers[1] = new gl::FrameBuffer(screenWidth,screenHeight);
        g_frameBuffers[1]->addTextureAttachment(GL_RGBA32F,GL_RGBA,GL_COLOR_ATTACHMENT0,false);
        if(!g_frameBuffers[1]->prepareBuffer({GL_COLOR_ATTACHMENT0})) throw std::runtime_error("OpenGL framebuffer initialization failed");
    }
...
void ClearOpenGl()
{
    ...
    delete g_frameBuffers[0];
    delete g_frameBuffers[1];
...
void RenderPrimaryQuad(glm::mat4& previousViewProj)
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, g_frameBuffers[g_frameBufferIdxCurrent]->getId());
    ...
    auto width = g_frameBuffers[g_frameBufferIdxCurrent]->getWidth();
    auto height = g_frameBuffers[g_frameBufferIdxCurrent]->getHeight();
    ...
    // Матрицы с которыми рисуется текущий кадр
    auto view = g_camera->getViewMatrix();
    auto proj = g_camera->getProjMatrix();
    auto projInverse = g_camera->getProjInverseMatrix();
    auto viewInverse = g_camera->getModelMatrix();

    // Передать FOV
    glUniform1f(g_shaderPathTracing->getUniformLocations()->camFov, 90.0f);
    // Передать размеры экрана
    glUniform2fv(g_shaderPathTracing->getUniformLocations()->screenSize, 1, glm::value_ptr(glm::vec2(static_cast<float>(width), static_cast<float>(height))));
    ...
    glUniformMatrix4fv(g_shaderPathTracing->getUniformLocations()->viewMatrix, 1, GL_FALSE, glm::value_ptr(view));
    // Передать матрицу модели камеры
    glUniformMatrix4fv(g_shaderPathTracing->getUniformLocations()->viewInverseMatrix, 1, GL_FALSE, glm::value_ptr(viewInverse));
    // Передать матрицу проекции
    glUniformMatrix4fv(g_shaderPathTracing->getUniformLocations()->projMatrix, 1, GL_FALSE, glm::value_ptr(proj));
    // Передать инвертированную матрицу проекции
    glUniformMatrix4fv(g_shaderPathTracing->getUniformLocations()->projInverseMatrix, 1, GL_FALSE, glm::value_ptr(projInverse));
    ...
    // Передать вспомогательный кадровый буфер предыдущего кадра
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D,g_frameBuffers[g_frameBufferIdxPrevious]->getTextureAttachments()[0]);
    glUniform1i(g_shaderPathTracing->getUniformLocations()->previousFrame,0);
    // Передать матрицу вида-проекции предыдущего кадра
    glUniformMatrix4fv(g_shaderPathTracing->getUniformLocations()->previousViewProjMatrix, 1, GL_FALSE, glm::value_ptr(previousViewProj));
    ...
    // Обновить матрицу вида-проекции предыдущего кадра (для следующего)
    previousViewProj = (proj * view);
}
...
void RenderFinalQuad(GLuint width, GLuint height)
{
    ...
    // Передать размеры экрана
    glUniform2fv(g_shaderPostProcess->getUniformLocations()->screenSize, 1, glm::value_ptr(glm::vec2(static_cast<float>(width), static_cast<float>(height))));
    // Передать цветовое вложение буфера в который последним осуществлялась запись
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D,g_frameBuffers[g_frameBufferIdxCurrent]->getTextureAttachments()[0]);
...
void SwitchFrameBuffers()
{
    std::swap(g_frameBufferIdxPrevious, g_frameBufferIdxCurrent);
}
